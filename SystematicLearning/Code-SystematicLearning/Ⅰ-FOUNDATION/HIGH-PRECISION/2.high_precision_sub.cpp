 ...
 * @Author  : Karry Ren
 * @Comment : Compute the subbing large number of str.
 * @LeetCode: None
*/
...
bool cmp(vector<int> &A, vector<int> &B) {
    ...
    // compare the length first. if not equal
    if (A.size() != B.size()) return A.size() > B.size();

    // if length equal, compare the num from top to down
...
vector<int> high_precision_sub(vector<int> &A, vector<int> &B) {
    ...
    for (int i = 0; i < A.size(); i++) { // the length of A must be higher than the length of B
        t = A[i] - t;
        if (i < B.size()) t -= B[i];
        if (t >= 0) { // 如果不存在借位现象就直接将这一位存储起来, 然后 t 置零
            C.push_back(t);
            t = 0; // 0 表示没有借位
        } else { // 如果存在借位现象, 就直接存 t + 10 (负数 + 10)
            C.push_back(t + 10);
            t = 1; // 1 表示存在借位
        }
    }
...
int main() {
    ...
    for (int i = b.size() - 1; i >= 0; i--) B.push_back(b[i] - '0'); // B = [9, 2, 3, 4, 5, 6]

    // do the high precision sub
    if (cmp(A, B)) {
